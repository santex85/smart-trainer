...
Написано **11 новых тестов** в `backend/tests/test_performance_optimizations.py`:

| Тест | Что проверяет |
|------|---------------|
| `test_intervals_sync_batch_workout_upsert` | Корректность формирования batch-строк для Intervals |
| `test_image_resize_async_offloads_to_threadpool` | Async ресайз через threadpool, результат корректен |
| `test_image_resize_sync_backward_compat` | Sync-версия ресайза работает для обратной совместимости |
| `test_compute_fitness_returns_none_when_no_workouts` | CTL/ATL/TSB = None при пустой БД |
| `test_compute_fitness_with_workouts` | CTL/ATL/TSB корректно считаются с mock-данными |
| `test_orchestrator_context_queries_called` | `_build_context` формирует корректный текст |
| `test_gzip_middleware_configured` | GZip middleware присутствует в FastAPI app |
| `test_db_pool_settings` | pool_size >= 10, max_overflow >= 20, recycle = 1800 |
| `test_strava_sync_prepares_batch` | Парсинг дат Strava активностей |
| `test_http_client_singleton` | HTTP-клиент — singleton |
| `test_concurrent_orchestrator_semaphore` | Семафор ограничивает параллелизм |

**Результат запуска:** Все 12 тестов (1 существующий + 11 новых) — **PASSED**.

```
tests/test_load_metrics.py::test_compute_fitness_returns_none_when_no_workouts PASSED
tests/test_performance_optimizations.py::test_intervals_sync_batch_workout_upsert PASSED
tests/test_performance_optimizations.py::test_image_resize_async_offloads_to_threadpool PASSED
tests/test_performance_optimizations.py::test_image_resize_sync_backward_compat PASSED
tests/test_performance_optimizations.py::test_compute_fitness_returns_none_when_no_workouts PASSED
tests/test_performance_optimizations.py::test_compute_fitness_with_workouts PASSED
tests/test_performance_optimizations.py::test_orchestrator_context_queries_called PASSED
tests/test_performance_optimizations.py::test_gzip_middleware_configured PASSED
tests/test_performance_optimizations.py::test_db_pool_settings PASSED
tests/test_performance_optimizations.py::test_strava_sync_prepares_batch PASSED
tests/test_performance_optimizations.py::test_http_client_singleton PASSED
tests/test_performance_optimizations.py::test_concurrent_orchestrator_semaphore PASSED
======================== 12 passed in 1.19s =========================
```
...
```
backend/app/services/intervals_sync.py     — batch upsert
backend/app/services/strava_sync.py        — batch upsert
backend/app/services/orchestrator.py       — asyncio.gather
backend/app/api/v1/chat.py                 — asyncio.gather
backend/app/main.py                        — concurrent orchestrator, GZip, Gemini init
backend/app/db/session.py                  — pool tuning
backend/app/services/image_resize.py       — async threadpool
backend/app/api/v1/nutrition.py            — async resize
backend/app/api/v1/photo.py                — async resize
backend/app/services/gemini_nutrition.py   — убран genai.configure
backend/app/services/gemini_photo_analyzer.py — убран genai.configure
backend/app/services/gemini_photo_classifier.py — убран genai.configure
backend/app/services/gemini_sleep_parser.py — убран genai.configure
frontend/src/screens/DashboardScreen.tsx   — React.memo, useMemo
backend/tests/test_performance_optimizations.py — 11 тестов
```
