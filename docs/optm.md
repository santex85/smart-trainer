...
## 3. Обнаруженные проблемы и целевые оптимизации

**Примечание:** Пункты 3.1–3.10 описывают **целевые** (рекомендуемые) оптимизации. В текущей кодовой базе они **реализованы**: GZip, ресайз в threadpool, genai в lifespan, pool_recycle, asyncio.gather в оркестраторе и чате, batch upsert для Intervals (wellness и workouts), batch upsert для Strava, параллельный планировщик с семафором, React.memo/useMemo в Dashboard.
